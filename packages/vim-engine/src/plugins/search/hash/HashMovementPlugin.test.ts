/**
 * Tests for HashMovementPlugin
 */

import { describe, it, expect } from 'vitest';
import { HashMovementPlugin } from './HashMovementPlugin';
import { VimState } from '../../../state/VimState';
import { CursorPosition } from '../../../state/CursorPosition';
import { TextBuffer } from '../../../state/TextBuffer';
import { ExecutionContext } from '../../../plugin/ExecutionContext';

describe('HashMovementPlugin', () => {
  it('should search backward for the word under cursor', () => {
    ...
    // Move cursor to second 'hello' (position 0,12)
    ...
    // Perform the hash action
    plugin.performAction(context);

    // Should have moved to the first 'hello' at column 0
  ...
  it('should extract complete word with underscores', () => {
    ...
    state.buffer.setContent(content);
    state.cursor = new CursorPosition(0, 20); // Position in middle of second 'test_variable'
    ...
    // Should have moved to the first occurrence
    expect(state.cursor.column).toBe(0);
  });

  it('should wrap around to find previous occurrence', () => {
    const state = new VimState();
    state.buffer = new TextBuffer();
    const plugin = new HashMovementPlugin();
  ...
  it('should match whole words only', () => {
    ...
    state.buffer.setContent(content);
    state.cursor = new CursorPosition(0, 29); // On last 'hello'
