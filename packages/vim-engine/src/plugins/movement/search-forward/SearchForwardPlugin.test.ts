/**
 * SearchForwardPlugin Unit Tests
 * Tests for the / key plugin (initiate forward search)
 */
import { describe, it, expect, beforeEach, vi } from 'vitest';
import { SearchForwardPlugin } from './SearchForwardPlugin';
...
describe('SearchForwardPlugin', () => {
  let plugin: SearchForwardPlugin;
  ...
  beforeEach(() => {
    ...
    plugin = new SearchForwardPlugin(searchInputManager);
    state = new VimState();
  ...
  describe('Metadata', () => {
    ...
    it('should have correct description', () => {
      expect(plugin.description).toBe('Initiate forward search (/ key)');
    });
  ...
  describe('Initialization', () => {
    it('should accept SearchInputManager in constructor', () => {
      ...
      const testPlugin = new SearchForwardPlugin(manager);
      expect(testPlugin).toBeInstanceOf(SearchForwardPlugin);
    });
  });

  describe('Action Behavior', () => {
    it('should call start with forward direction when executed in NORMAL mode', () => {
      const startSpy = vi.spyOn(searchInputManager, 'start');
      context.setMode(VIM_MODE.NORMAL);

      plugin.execute(context);

      expect(startSpy).toHaveBeenCalledWith('forward');
      expect(startSpy).toHaveBeenCalledTimes(1);
    });

    it('should call start with forward direction when executed in VISUAL mode', () => {
      const startSpy = vi.spyOn(searchInputManager, 'start');
    ...
    it('should set search direction to forward in search input manager', () => {
      context.setMode(VIM_MODE.NORMAL);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle execution with existing search state', () => {
      searchInputManager.start('backward');
