/**
 * SearchPrevPlugin Unit Tests
 * Tests for the N key plugin (jump to previous search result)
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { SearchPrevPlugin } from './SearchPrevPlugin';
import { ExecutionContext } from '../../../plugin/ExecutionContext';
import { VimState } from '../../../state/VimState';
import { VIM_MODE } from '../../../state/VimMode';
import { CursorPosition } from '../../../state/CursorPosition';

describe('SearchPrevPlugin', () => {
  let plugin: SearchPrevPlugin;
  let state: VimState;
  let context: ExecutionContext;

  beforeEach(() => {
    plugin = new SearchPrevPlugin();
    state = new VimState();
  ...
  describe('Metadata', () => {
    it('should have correct name', () => {
      expect(plugin.name).toBe('movement-search-prev');
    });
    ...
    it('should have correct description', () => {
      expect(plugin.description).toBe('Jump to previous search result (N key)');
    });
  ...
  describe('Action Behavior', () => {
    ...
    it('should navigate to previous match when search exists and matches are available', () => {
      // Set up search state with forward direction
      ...
      state.currentMatchPosition = new CursorPosition(0, 10);

      context.setMode(VIM_MODE.NORMAL);

      plugin.execute(context);

      // Should move to previous match (0, 0) - opposite of forward is backward
      expect(context.getCursor().equals(new CursorPosition(0, 0))).toBe(true);
      expect(state.currentMatchPosition?.equals(new CursorPosition(0, 0))).toBe(true);
    });

    it('should update currentMatchPosition when navigating to previous match', () => {
      state.lastSearchPattern = 'pattern';
      state.lastSearchDirection = 'forward';
      state.searchMatches = [
        new CursorPosition(0, 0),
    ...
    it('should do nothing when no more matches available (no wrap)', () => {
      ...
      state.currentMatchPosition = new CursorPosition(0, 0); // At first match
    ...
    it('should use backward direction when lastSearchDirection is forward', () => {
      state.lastSearchPattern = 'test';
      state.lastSearchDirection = 'forward';
      state.searchMatches = [
        new CursorPosition(0, 0),
      ...
      // Should go to previous match (opposite of forward)
      expect(context.getCursor().equals(new CursorPosition(0, 5))).toBe(true);
    });

    it('should use forward direction when lastSearchDirection is backward', () => {
      state.lastSearchPattern = 'test';
      ...
      // Should go to next match (opposite of backward is forward)
    ...
    it('should handle navigation when cursor is not on a match', () => {
      ...
      // When searching backward from before the first match, no match is found
      // because there are no matches before the cursor position
      expect(context.getCursor().equals(new CursorPosition(0, 0))).toBe(true);
    });
    ...
    it('should not wrap when at first match with wrap disabled', () => {
      state.lastSearchPattern = 'test';
      state.lastSearchDirection = 'forward';
      state.searchMatches = [
        new CursorPosition(0, 0),
  ...
  describe('Edge Cases', () => {
    it('should handle navigation with matches on multiple lines', () => {
      ...
      state.currentMatchPosition = new CursorPosition(2, 10);
    ...
    it('should handle when currentMatchPosition is null but cursor is on a match', () => {
      ...
      state.currentMatchPosition = null;
      state.cursor = new CursorPosition(0, 5); // Cursor on second match

      context.setMode(VIM_MODE.NORMAL);

      plugin.execute(context);

      // Should find previous match since cursor is on a match
      expect(context.getCursor().equals(new CursorPosition(0, 0))).toBe(true);
    });
