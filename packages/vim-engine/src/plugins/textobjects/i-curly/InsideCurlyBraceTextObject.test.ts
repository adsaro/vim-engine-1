/**
 * InsideCurlyBraceTextObject Tests
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { InsideCurlyBraceTextObject } from './InsideCurlyBraceTextObject';
...
describe('InsideCurlyBraceTextObject', () => {
  let plugin: InsideCurlyBraceTextObject;
  let context: ExecutionContext;
  let state: VimState;

  beforeEach(() => {
    plugin = new InsideCurlyBraceTextObject();
    state = new VimState('');
  ...
  describe('plugin metadata', () => {
    ...
    it('should have correct description', () => {
      expect(plugin.description).toBe('Inside curly brace text object (i{)');
    });
  ...
  describe('getWordBoundaries', () => {
    it('should find content inside curly braces on same line', () => {
    ...
    it('should find content when cursor is right after opening brace', () => {
      state.buffer = new TextBuffer('{hello world}');
    ...
    it('should find content when cursor is right before closing brace', () => {
      state.buffer = new TextBuffer('{hello world}');
    ...
    it('should find innermost pair when cursor is in nested braces', () => {
      ...
      expect(boundaries).not.toBeNull();
      // Cursor is in "content", so it should find the outer braces
    ...
    it('should handle nested braces - cursor in inner braces', () => {
      ...
      expect(boundaries).not.toBeNull();
      // Cursor is in "inner", so it should find the inner braces
      // The inner braces start at position 7 ({ o u t e r _ {)
      // After the '{', the inner content starts at position 8
      // But we're returning boundaries relative to the inner braces
    ...
    it('should return null when cursor is before the opening brace', () => {
      state.buffer = new TextBuffer('hello {world}');
    ...
    it('should return null when no opening brace exists', () => {
      state.buffer = new TextBuffer('hello world');
    ...
    it('should return null when no closing brace exists', () => {
      state.buffer = new TextBuffer('{hello world');
      state.cursor = new CursorPosition(0, 5);
    ...
    it('should return null for empty braces', () => {
      state.buffer = new TextBuffer('{}');
      state.cursor = new CursorPosition(0, 1); // Between braces

      const boundaries = plugin.getWordBoundaries(context);

      // Empty braces - there's no content to select
      expect(boundaries).toBeNull();
    });

    it('should handle braces with only whitespace', () => {
      state.buffer = new TextBuffer('{   }');
    ...
    it('should handle braces across multiple lines with content', () => {
      state.buffer = new TextBuffer([
        'function test() {',
        '  const x = 1;',
        '  const y = 2;',
    ...
    it('should find brace on previous line', () => {
      state.buffer = new TextBuffer([
        '{',
        '  line 2',
        '  line 3',
    ...
    it('should handle cursor at start of buffer on brace', () => {
      state.buffer = new TextBuffer('{content}');
      state.cursor = new CursorPosition(0, 0); // On '{'

      const boundaries = plugin.getWordBoundaries(context);

      // Cursor is on the opening brace, not strictly inside
      // So it should return null
      expect(boundaries).toBeNull();
    });

    it('should return null when cursor is after the closing brace', () => {
      state.buffer = new TextBuffer('{content}');
      state.cursor = new CursorPosition(0, 9); // After '}'

      const boundaries = plugin.getWordBoundaries(context);

      // Cursor is after the closing brace, not strictly inside
      expect(boundaries).toBeNull();
    });
